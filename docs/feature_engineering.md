...
* **Tabular baseline training**: Reads the existing feature table and dynamically identifies all graph-derived columns (`fastrp_0` through `fastrp_127` and `community_id`) to exclude; trains the same three classifiers under `/Shared/graph_feature_forge/tabular_only_baseline` using only the two tabular features
