...
Raw data files and the generators that produce them. The generators are a
self-contained project with their own `pyproject.toml` — they do not affect
the parent pipeline wheel or its dependencies.
...
└── pyproject.toml        # Generator dependencies (isolated from parent)
```
...
uv run csv_generator

# Override counts via env vars (GEN_ prefix)
GEN_NUM_CUSTOMERS=1000 GEN_NUM_BANKS=100 uv run csv_generator
```
